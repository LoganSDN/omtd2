...
function XMatchClient(requestURL, activeRequestLimit) {
    "use strict";
    ...
    function findRequestURL(requestURL) {
        if (!requestURL || typeof requestURL !== "string") {
            return findBaseURL() + "rs/XMatch/";
        } else if(startsWith(requestURL, "http")) {
            return requestURL;
        } else {
            return findBaseURL() + "rs/XMatch/" + requestURL + "/";
        }
    }
     ...
     */
    this.matchTrack = function (request , handler, timeout) {
        return sendRequest("matchTrack", request, handler, timeout || this.matchTrack.timeout);
    };
     ...
     * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
     */
    this.startMatchTrack = function (request , handler, timeout) {
        return sendRequest("startMatchTrack", request, handler, timeout || this.startMatchTrack.timeout);
    };
     ...
     */
    this.runMatchTrack = function (request , handler, watchRequest, onUpdate, retryOptions) {
        return runJob(client.startMatchTrack, request, client.fetchTrackResponse, handler, watchRequest, onUpdate, retryOptions);
    };
     ...
     * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
     */
    this.matchPositions = function (request , handler, timeout) {
        return sendRequest("matchPositions", request, handler, timeout || this.matchPositions.timeout);
    };
     ...
     * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
     */
    this.fetchTrackResponse = function (id, onResult, timeout) {
        return sendRequest("fetchTrackResponse", { id: id }, onResult, timeout || client.fetchTrackResponse.timeout);
    };
