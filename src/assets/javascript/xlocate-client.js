...
function XLocateClient(requestURL, activeRequestLimit) {
     ...
     */
    this.searchLocations = function (request , handler, timeout) {
        return sendRequest("searchLocations", request, handler, timeout || this.searchLocations.timeout);
    };
     ...
     * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
     */
    this.searchLocationsInBulk = function (request , handler, timeout) {
        return sendRequest("searchLocationsInBulk", request, handler, timeout || this.searchLocationsInBulk.timeout);
    };
     ...
     * @throws {object} only in the synchroneous mode: the error message from the server, or an Error object if a CORS request would be needed for an older IE browser.
     */
    this.startSearchLocationsInBulk = function (request , handler, timeout) {
        return sendRequest("startSearchLocationsInBulk", request, handler, timeout || this.startSearchLocationsInBulk.timeout);
    };
     ...
     */
    this.runSearchLocationsInBulk = function (request , handler, watchRequest, onUpdate, retryOptions) {
        return runJob(client.startSearchLocationsInBulk, request, client.fetchLocationsBulkResponse, handler, watchRequest, onUpdate, retryOptions);
    };
